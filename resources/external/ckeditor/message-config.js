...
CKEDITOR.editorConfig = function (config) {
    ...
    //config.extraPlugins = 'wittyemailembedwit';
    config.removePlugins = 'annotation,wittyembedwit';
    ...
    config.toolbar_WittyToolbar = [
        ...
        {name: 'justify', items: ['JustifyBlock', '-']},
...
CKEDITOR.on( 'dialogDefinition', function( ev ) {
    ...
    if ( dialogName == 'acronym' ) {
        dialogDefinition.onShow = function () {
            var editor = CKEDITOR.instances.editable;
            var zws = '\u200B';
            var acronymText = editor.getSelection().getSelectedText();
            if(editor && acronymText) {
                //this.getContentElement( 'general', 'acronymText' ).setValue( acronymText );
                ...
                acrospan.setAttribute('data-default', ' ' );
